defmodule AccessGuardian.Access.AccessRequest do
  ...
  attributes do
    ...
    attribute :status, :atom do
      constraints one_of: [:pending_approval, :approved, :provisioning, :granted, :rejected, :denied]
  ...
  actions do
    ...
    create :create do
      accept [:organization_id, :affected_user_id, :requested_by_id,
              :application_id, :request_reason, :entitlements]
    ...
    update :deny do
      ...
      change fn changeset, _ctx ->
        changeset
        |> Ash.Changeset.force_change_attribute(:denied_by_id, Ash.Changeset.get_argument(changeset, :denier_id))
        |> Ash.Changeset.force_change_attribute(:deny_reason, Ash.Changeset.get_argument(changeset, :reason))
      end
    end
    ...
    update :complete_provisioning do
      ...
      change fn changeset, _ctx ->
        changeset
        |> Ash.Changeset.force_change_attribute(:adapter_type, Ash.Changeset.get_argument(changeset, :adapter_type))
        |> Ash.Changeset.force_change_attribute(:external_account_id, Ash.Changeset.get_argument(changeset, :external_account_id))
      end
    ...
    update :fail_provisioning do
      ...
      change fn changeset, _ctx ->
        changeset
        |> Ash.Changeset.force_change_attribute(:adapter_type, Ash.Changeset.get_argument(changeset, :adapter_type))
        |> Ash.Changeset.force_change_attribute(:reject_reason, Ash.Changeset.get_argument(changeset, :error_reason))
      end
    end
    ...
    read :by_org do
      ...
      prepare build(sort: [inserted_at: :desc], load: [:affected_user, :requested_by, :application])
    end
